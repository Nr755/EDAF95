...
rowUnits = [cross [r] cols4 | r <- rows4]
colUnits = [cross rows4 [c] | c <- cols4]
...
printSudoku board = do
  ...
  mapM_
    ( \r -> do
        let rowCells =
              [ let cellId = [r, c]
                    -- Look up the raw value from the board.
                    valStr = case lookup cellId board of
                      Just 0 -> "."
                      Just n -> show n
                      Nothing -> "."
                    -- Check its possibilities from boardNumbers.
